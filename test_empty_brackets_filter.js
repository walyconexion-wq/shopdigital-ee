...
async function testEmptyBracketsFilter() {
  try {
    ...
    // Set mapped text to use empty brackets
    if (mod2.mapper.contents && mod2.mapper.contents[0] && mod2.mapper.contents[0].parts && mod2.mapper.contents[0].parts[0]) {
      mod2.mapper.contents[0].parts[0].text = "{{1.entry[].changes[].value.messages[].text.body}}";
    }

    // Set filter to check if messages text body exists (operator: exists) using empty brackets
    mod2.filter = {
      label: "Only on message",
      conditions: [
        [
          {
            a: "{{1.entry[].changes[].value.messages[].text.body}}",
            o: "exists"
          }
        ]
      ]
    ...
    // 3. Find Module 5 and update its `to` mapper using empty brackets
    const mod5 = bp.flow.find(m => m.id === 5);
    if (!mod5) throw new Error("Module 5 not found");
    mod5.mapper.to = "{{1.entry[].changes[].value.messages[].from}}";

    console.log("Patching scenario with empty brackets filter...");
                ...
                messages: [
                  {
                    ...
                    text: { body: "Hola, probando con empty brackets filter y sin index" },
                    type: "text"
                  }
                ]
              },
