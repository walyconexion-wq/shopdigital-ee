...
const EnterpriseMasterPanelPage: React.FC = () => {
    ...
    const CYAN = '#06b6d4'; const BLUE = '#3b82f6';


    // ============================
    // CONSOLA CYAN PRINCIPAL
    // ============================
    ...
    const quickLinks = [
        ...
        { title: 'Directorio Industrial', desc: 'Portal público de rubros', path: `/empresas`, icon: <Factory size={16} />, color: 'cyan' },
        { title: 'Gestión de Empresas', desc: `Control · ${currentProvince.name}`, path: `/esteban-echeverria/embajador/empresas?provincia=${provinciaParam}`, icon: <Settings size={16} />, color: 'cyan' },
        { title: 'Nueva Empresa', desc: `Alta B2B · ${currentProvince.name}`, path: `/esteban-echeverria/embajador/empresas/nueva?provincia=${provinciaParam}`, icon: <FileText size={16} />, color: 'cyan' },
        { title: '📢 Marketing Industrial', desc: 'Automatización y Campañas', path: `/empresas/marketing-inteligente`, icon: <Megaphone size={16} />, color: 'cyan' },
    ...
    return (
        <div className="min-h-screen bg-[#020617] tech-grid-bg text-white pb-24 relative overflow-hidden selection:bg-cyan-500/30">
            {/* Background Amber HUD */}
            <div className="fixed inset-0 pointer-events-none z-0">
                <div className="absolute top-0 right-0 w-[400px] h-[400px] rounded-full blur-[100px]" style={{ backgroundColor: hexToRgba(CYAN, 0.08) }} />
                <div className="absolute bottom-0 left-0 w-[300px] h-[300px] rounded-full blur-[100px]" style={{ backgroundColor: hexToRgba(CYAN, 0.05) }} />
            ...
            <div className="bg-zinc-900/50 backdrop-blur-md pt-8 pb-6 px-6 flex flex-col items-center border-b border-cyan-500/20 mb-6 sticky top-0 z-50">
                <button onClick={() => { playNeonClick(); navigate(`/empresas`); }}
                    className="self-start mb-4 w-10 h-10 rounded-2xl bg-cyan-500/10 flex items-center justify-center text-cyan-400 border border-cyan-400/30 hover:bg-cyan-500/20 transition-all">
                    <ChevronLeft size={20} />
                </button>
            ...
            <div className="px-6 space-y-6 relative z-10 max-w-lg mx-auto">
                ...
                <div className="glass-card-neon bg-gradient-to-r from-cyan-500/10 to-cyan-500/5 border border-cyan-400/40 rounded-2xl p-5 relative overflow-hidden group shadow-[0_0_20px_rgba(6,182,212,0.15)]">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-xl bg-cyan-500/20 border border-cyan-400/30 flex items-center justify-center text-cyan-400 shadow-[0_0_15px_rgba(6,182,212,0.3)] animate-pulse">
                            ...
                            <div>
                                ...
                                <p className="text-[8px] text-cyan-400/80 uppercase tracking-widest mt-1.5 font-bold italic">
                        ...
                        <button
                            ...
                            className="px-4 py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-black text-[10px] font-black uppercase tracking-widest transition-all duration-300 shadow-[0_0_20px_rgba(6,182,212,0.4)] active:scale-95 hover:scale-105"
                        >
                            Ingresar
                        </button>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="glass-card-neon bg-cyan-500/5 border-cyan-500/20 rounded-2xl p-4 flex flex-col items-center gap-1">
                        <TrendingUp size={20} className="text-cyan-400" />
                        <span className="text-[28px] font-[1000] text-white">{enterprises.length}</span>
                        <span className="text-[8px] font-black text-cyan-400/60 uppercase tracking-widest">Total Empresas</span>
                    </div>
                    <div className="glass-card-neon bg-green-500/5 border-green-500/20 rounded-2xl p-4 flex flex-col items-center gap-1">
                    ...
                    <div className="glass-card-neon bg-red-500/5 border-red-500/20 rounded-2xl p-4 flex flex-col items-center gap-1">
                ...
                <div className="glass-card-neon bg-white/[0.02] border-cyan-500/15 rounded-2xl p-5">
                    ...
                    <div className="flex gap-3">
                        <div className="flex-1 bg-cyan-500/10 rounded-xl p-3 border border-cyan-500/20 text-center">
                            <Globe size={16} className="text-cyan-400 mx-auto mb-1" />
                            <span className="text-[20px] font-[1000] text-white block">{nationalCount}</span>
                            <span className="text-[7px] font-black text-cyan-400/50 uppercase tracking-widest">Nacional 🌎</span>
                        </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {ENTERPRISE_CATEGORIES.map(cat => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`text-[14px] font-[1000] ${hasEnterprises ? 'text-cyan-400' : 'text-white/20'}`}>
                                        {count}
                                    </div>
                        ...
                        {quickLinks.map((link, i) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full bg-zinc-900/50 border border-cyan-500/15 rounded-xl p-3.5 flex items-center gap-3 ${c.hoverBg} hover:border-cyan-500/30 transition-all active:scale-[0.98] group`}
                                >
                    ...
                    <div className="glass-card-neon bg-white/[0.02] border-cyan-500/15 rounded-2xl p-5">
                        ...
                        <div className="space-y-2">
                            {activeEnterprises.map(ent => (
                                <div key={ent.id} className="bg-zinc-900/50 border border-cyan-500/10 rounded-xl p-3 flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-lg overflow-hidden bg-black/40 border border-cyan-500/20 flex-shrink-0">
                                        {ent.bannerImage ? (
                                            <img src={ent.bannerImage} alt={ent.name} className="w-full h-full object-cover" />
                                        ) : (
                                            <Factory size={16} className="text-cyan-500/30 m-auto mt-2.5" />
                                        )}
                                    </div>
            ...
            {/* Footer */}
            <footer className="w-full flex flex-col items-center gap-2 pt-8 pb-6 mt-8 border-t border-cyan-500/10 relative z-10">
