...
export const SystemsBunkerPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (!isAuthorized && !intrusionRegistered) {
            ...
            registrarIntrusionBunker((user?.email || 'anonimo') + ' (Bunker: Sistemas)');
        }
    ...
    const handleSend = async () => {
        ...
        const currentSysSummary = `
KPIs DE SISTEMAS E INFRAESTRUCTURA:
- Carga de CPU Servidor: 18% promedio
- Uptime Mensual: 99.98%
- Registros en Base de Datos: 550,000 documentos
- Latencia de API Promedio: 45ms
    ...
    if (authLoading) {
        return (
            <div className="min-h-screen bg-black flex flex-col items-center justify-center">
                ...
                <p className="text-[10px] text-white/30 uppercase tracking-[0.3em] font-bold">Verificando Credenciales de Sistemas...</p>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 relative overflow-hidden">
                ...
                <p className="text-white/40 text-[11px] text-center max-w-xs leading-relaxed">
                    Acceso exclusivo para Sistemas o Dirección General. Su intento de ingreso con el correo {user?.email || 'Anónimo'} ha sido registrado.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="h-screen overflow-y-auto overflow-x-hidden relative z-10 w-full bg-[#05050A] text-white font-sans flex flex-col selection:bg-indigo-500/30">
            {/* Background Camaleón */}
            <div className="fixed inset-0 pointer-events-none z-0">
                <div className="absolute top-[-20%] left-[-10%] w-[50vw] h-[50vw] bg-indigo-600/10 rounded-full blur-[120px]" />
            ...
            <header className="relative z-10 bg-black/60 backdrop-blur-md border-b border-white/5 py-4 px-6 flex items-center justify-between shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-[14px] font-[1000] uppercase tracking-[0.2em] text-indigo-400 flex items-center gap-2">
                            <Anchor size={14} className="text-indigo-400" /> BÚNKER DE SISTEMAS E INFRAESTRUCTURA
                        </h1>
                        <p className="text-[8px] text-white/50 tracking-[0.3em] font-bold uppercase mt-1">Terminal de Sistemas · ShopDigital</p>
                    </div>
                </div>
                <div className="flex items-center gap-4">
            ...
            <main className="flex-1 relative z-10 flex flex-col xl:flex-row w-full max-w-[1600px] mx-auto p-4 md:p-6 gap-6 min-h-[calc(100vh-80px)] pb-20">
                ...
                <div className="flex-[3] flex flex-col gap-6">
                    {/* Artillería */}
                    <div className="bg-black/40 border border-indigo-500/20 rounded-3xl p-6 backdrop-blur-md shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="bg-gradient-to-br from-indigo-900/10 to-black border border-indigo-500/20 rounded-2xl p-5 shadow-[0_0_15px_rgba(99,102,241,0.05)]">
                            <h3 className="text-[10px] text-indigo-400 font-black uppercase tracking-widest mb-1">Carga CPU</h3>
                            <p className="text-2xl font-[1000] text-white">18%</p>
                        </div>
                        <div className="bg-gradient-to-br from-indigo-900/10 to-black border border-indigo-500/20 rounded-2xl p-5 shadow-[0_0_15px_rgba(99,102,241,0.05)]">
                            <h3 className="text-[10px] text-indigo-400 font-black uppercase tracking-widest mb-1">Uptime</h3>
                            <p className="text-2xl font-[1000] text-white">99.98%</p>
                        </div>
                        <div className="bg-gradient-to-br from-indigo-900/10 to-black border border-indigo-500/20 rounded-2xl p-5 shadow-[0_0_15px_rgba(99,102,241,0.05)]">
                            <h3 className="text-[10px] text-indigo-400 font-black uppercase tracking-widest mb-1">DB Records</h3>
                            <p className="text-2xl font-[1000] text-white">550K</p>
                        </div>
                        <div className="bg-gradient-to-br from-indigo-900/10 to-black border border-indigo-500/20 rounded-2xl p-5 shadow-[0_0_15px_rgba(99,102,241,0.05)]">
                            <h3 className="text-[10px] text-indigo-400 font-black uppercase tracking-widest mb-1">Latencia API</h3>
                            <p className="text-2xl font-[1000] text-white">45ms</p>
                        </div>
                    </div>

                    {/* Reporte Operativo */}
                    <div className="bg-black/20 border border-white/5 rounded-3xl p-6 backdrop-blur-md flex-1">
                        <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-white/60 mb-4">Estado de Servidores</h3>
                        <div className="space-y-3">
                            <div className="p-3 bg-white/[0.02] border border-white/5 rounded-xl flex items-center justify-between text-[11px]">
                                <span className="text-white/50 uppercase">Firestore Database Health</span>
                                <span className="text-green-400 font-bold uppercase">Excelente (0% cuellos de botella)</span>
                            </div>
                            <div className="p-3 bg-white/[0.02] border border-white/5 rounded-xl flex items-center justify-between text-[11px]">
                                <span className="text-white/50 uppercase">Certificados SSL</span>
                                <span className="text-green-400 font-bold uppercase">Válidos</span>
                            </div>
                            <div className="p-3 bg-white/[0.02] border border-white/5 rounded-xl flex items-center justify-between text-[11px]">
                                <span className="text-white/50 uppercase">Webhook Vercel & Meta API</span>
                                <span className="text-indigo-400 font-bold uppercase">Sintonizado</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-[2] w-full bg-black/80 backdrop-blur-2xl border border-indigo-500/30 rounded-3xl overflow-hidden flex flex-col relative shadow-[0_0_40px_rgba(99,102,241,0.15)] h-[650px] shrink-0">
                    <div className="bg-gradient-to-r from-indigo-900/50 to-violet-950/50 p-4 border-b border-indigo-500/30 flex items-center gap-4">
                        <div className="relative">
                            <div className="w-12 h-12 rounded-2xl bg-black border border-indigo-500/50 flex items-center justify-center shadow-[0_0_15px_rgba(99,102,241,0.4)]">
                    ...
                    <div className="flex-1 overflow-y-auto p-5 pb-20 space-y-4 no-scrollbar">
                        {ariMsgs.map((msg, idx) => (
                            <div key={idx} className={`flex ${msg.role === 'ari' ? 'justify-start' : 'justify-end'}`}>
                                <div className={`max-w-[85%] rounded-2xl p-4 text-[13px] leading-relaxed ${
                                    msg.role === 'ari' 
                                    ? 'bg-indigo-950/20 border border-indigo-500/30 text-white rounded-tl-sm shadow-[0_5px_15px_rgba(99,102,241,0.1)]' 
                        ...
                        {isThinking && (
                            <div className="flex justify-start">
                                <div className="max-w-[85%] rounded-2xl p-4 bg-indigo-950/20 border border-indigo-500/30 text-white rounded-tl-sm flex items-center gap-2">
