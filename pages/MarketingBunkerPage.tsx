...
export const MarketingBunkerPage: React.FC = () => {
    ...
    const [ariMsgs, setAriMsgs] = useState([
        { role: 'ari' as 'ari' | 'user', text: 'Búnker de Marketing activo. Frecuencias publicitarias sintonizadas. ¿Qué campaña empujamos hoy, Jefe?' }
    ]);
    ...
    const handleChatFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const path = `bunker_chat/marketing/${Date.now()}_${file.name}`;
    ...
    const isAuthorized = user?.email?.trim().toLowerCase() === 'walyconexion@gmail.com' ||
                         user?.email?.trim().toLowerCase() === 'marketing@shopdigital.ar' ||
    ...
    useEffect(() => {
        ...
        if (!isAuthorized && !intrusionRegistered) {
            ...
            registrarIntrusionBunker((user?.email || 'anonimo') + ' (Bunker: Marketing)');
        }
    ...
    const handleSend = async () => {
        ...
        const currentMktSummary = `
KPIs DE MARKETING Y EXPANSIÓN:
- Campañas Activas: 12 campañas en distribución
- Alcance Regional: 180,000 personas impactadas
- Costo Adquisición (CAC): $1,200 ARS por comercio
- Conversión de Embudo: 8.5%
    ...
    if (authLoading) {
        return (
            <div className="min-h-screen bg-black flex flex-col items-center justify-center">
                ...
                <p className="text-[10px] text-white/30 uppercase tracking-[0.3em] font-bold">Verificando Credenciales de Marketing...</p>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 relative overflow-hidden">
                ...
                <p className="text-white/40 text-[11px] text-center max-w-xs leading-relaxed">
                    Acceso exclusivo para el área de Marketing o Dirección General. Su intento de ingreso con el correo {user?.email || 'Anónimo'} ha sido registrado.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="h-screen overflow-y-auto overflow-x-hidden relative z-10 w-full bg-[#050A07] text-white font-sans flex flex-col selection:bg-emerald-500/30">
            {/* Background Camaleón */}
            ...
            <header className="relative z-10 bg-black/60 backdrop-blur-md border-b border-white/5 py-4 px-6 flex items-center justify-between shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-[14px] font-[1000] uppercase tracking-[0.2em] text-emerald-400 flex items-center gap-2">
                            <Anchor size={14} className="text-emerald-400" /> BÚNKER DE MARKETING Y EXPANSIÓN
                        </h1>
                        <p className="text-[8px] text-white/50 tracking-[0.3em] font-bold uppercase mt-1">Terminal de Marketing · ShopDigital</p>
                    </div>
                </div>
                <div className="flex items-center gap-4">
                    ...
                    {temp !== null && (
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg">
                            <span className="text-[10px] font-black text-emerald-300 flex items-center gap-1.5">
            ...
            <main className="flex-1 relative z-10 flex flex-col xl:flex-row w-full max-w-[1600px] mx-auto p-4 md:p-6 gap-6 min-h-[calc(100vh-80px)] pb-20">
                ...
                <div className="flex-[3] flex flex-col gap-6">
                    {/* Artillería */}
                    <div className="bg-black/40 border border-emerald-500/20 rounded-3xl p-6 backdrop-blur-md shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                        <h2 className="text-[12px] font-black uppercase tracking-[0.25em] text-emerald-400 mb-6 flex items-center gap-2">
                            <Cpu size={14} /> Artillería del Búnker (Herramientas Operativas)
                        </h2>
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                            <a href="https://mail.google.com/mail/u/0/#inbox" target="_blank" rel="noopener noreferrer" className="bg-gradient-to-br from-emerald-950/20 to-black border border-emerald-500/30 hover:border-emerald-400 p-4 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all active:scale-95 hover:shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                                <span className="text-emerald-400 text-xs font-bold uppercase tracking-widest">Gmail</span>
                                <ExternalLink size={12} className="text-white/40" />
                            </a>
                            <a href="https://trello.com" target="_blank" rel="noopener noreferrer" className="bg-gradient-to-br from-emerald-950/20 to-black border border-emerald-500/30 hover:border-emerald-400 p-4 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all active:scale-95 hover:shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                                <span className="text-emerald-400 text-xs font-bold uppercase tracking-widest">Trello</span>
                                <ExternalLink size={12} className="text-white/40" />
                            </a>
                            <a href="https://canva.com" target="_blank" rel="noopener noreferrer" className="bg-gradient-to-br from-emerald-950/20 to-black border border-emerald-500/30 hover:border-emerald-400 p-4 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all active:scale-95 hover:shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                                <span className="text-emerald-400 text-xs font-bold uppercase tracking-widest">Canva</span>
                                <ExternalLink size={12} className="text-white/40" />
                            </a>
                            <button onClick={() => navigate(`/${townId}/marketing-inteligente`)} className="bg-gradient-to-br from-emerald-950/20 to-black border border-emerald-500/30 hover:border-emerald-400 p-4 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all active:scale-95 hover:shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                                <span className="text-emerald-400 text-xs font-bold uppercase tracking-widest">Marketing</span>
                                <ArrowUpRight size={12} className="text-white/40" />
                            </button>
                            <a href="https://keep.google.com" target="_blank" rel="noopener noreferrer" className="bg-gradient-to-br from-emerald-950/20 to-black border border-emerald-500/30 hover:border-emerald-400 p-4 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all active:scale-95 hover:shadow-[0_0_15px_rgba(16,185,129,0.2)]">
                                <span className="text-emerald-400 text-xs font-bold uppercase tracking-widest">Cuaderno</span>
                                <ExternalLink size={12} className="text-white/40" />
                            </a>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="bg-gradient-to-br from-emerald-900/10 to-black border border-emerald-500/20 rounded-2xl p-5 shadow-[0_0_15px_rgba(16,185,129,0.05)]">
                            <h3 className="text-[10px] text-emerald-400 font-black uppercase tracking-widest mb-1">Campañas Activas</h3>
                            <p className="text-2xl font-[1000] text-white">12</p>
                        </div>
                        <div className="bg-gradient-to-br from-emerald-900/10 to-black border border-emerald-500/20 rounded-2xl p-5 shadow-[0_0_15px_rgba(16,185,129,0.05)]">
                            <h3 className="text-[10px] text-emerald-400 font-black uppercase tracking-widest mb-1">Alcance Total</h3>
                            <p className="text-2xl font-[1000] text-white">180K</p>
                        </div>
                        <div className="bg-gradient-to-br from-emerald-900/10 to-black border border-emerald-500/20 rounded-2xl p-5 shadow-[0_0_15px_rgba(16,185,129,0.05)]">
                            <h3 className="text-[10px] text-emerald-400 font-black uppercase tracking-widest mb-1">CAC Promedio</h3>
                            <p className="text-2xl font-[1000] text-white">$1.2K</p>
                        </div>
                        <div className="bg-gradient-to-br from-emerald-900/10 to-black border border-emerald-500/20 rounded-2xl p-5 shadow-[0_0_15px_rgba(16,185,129,0.05)]">
                            <h3 className="text-[10px] text-emerald-400 font-black uppercase tracking-widest mb-1">Conversión</h3>
                            <p className="text-2xl font-[1000] text-white">8.5%</p>
                        </div>
                    </div>

                    {/* Reporte de Campañas */}
                    <div className="bg-black/20 border border-white/5 rounded-3xl p-6 backdrop-blur-md flex-1">
                        <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-white/60 mb-4">Estado de Canales y Difusiones</h3>
                        <div className="space-y-3">
                            <div className="p-3 bg-white/[0.02] border border-white/5 rounded-xl flex items-center justify-between text-[11px]">
                                <span className="text-white/50 uppercase">Campaña Comerciantes B2B</span>
                                <span className="text-emerald-400 font-bold uppercase">Corriendo</span>
                            </div>
                            <div className="p-3 bg-white/[0.02] border border-white/5 rounded-xl flex items-center justify-between text-[11px]">
                                <span className="text-white/50 uppercase">Foco Audiencia Calle B2C</span>
                                <span className="text-emerald-400 font-bold uppercase">Optimizado</span>
                            </div>
                            <div className="p-3 bg-white/[0.02] border border-white/5 rounded-xl flex items-center justify-between text-[11px]">
                                <span className="text-white/50 uppercase">Pauta en Redes Locales</span>
                                <span className="text-emerald-300 font-bold uppercase">Planificada</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-[2] w-full bg-black/80 backdrop-blur-2xl border border-emerald-500/30 rounded-3xl overflow-hidden flex flex-col relative shadow-[0_0_40px_rgba(16,185,129,0.15)] h-[650px] shrink-0">
                    <div className="bg-gradient-to-r from-emerald-900/50 to-teal-950/50 p-4 border-b border-emerald-500/30 flex items-center gap-4">
                        <div className="relative">
                            <div className="w-12 h-12 rounded-2xl bg-black border border-emerald-500/50 flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.4)]">
                        ...
                        <div>
                            ...
                            <p className="text-[8px] text-emerald-300 font-bold tracking-[0.3em] uppercase">Analista de Marketing · En línea</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-5 pb-20 space-y-4 no-scrollbar">
                        {ariMsgs.map((msg, idx) => (
                            <div key={idx} className={`flex ${msg.role === 'ari' ? 'justify-start' : 'justify-end'}`}>
                                <div className={`max-w-[85%] rounded-2xl p-4 text-[13px] leading-relaxed ${
                                    msg.role === 'ari' 
                                    ? 'bg-emerald-950/20 border border-emerald-500/30 text-white rounded-tl-sm shadow-[0_5px_15px_rgba(16,185,129,0.1)]' 
                        ...
                        {isThinking && (
                            <div className="flex justify-start">
                                <div className="max-w-[85%] rounded-2xl p-4 bg-emerald-950/20 border border-emerald-500/30 text-white rounded-tl-sm flex items-center gap-2">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black via-black/90 to-transparent">
                        <div className="bg-black border border-emerald-500/40 rounded-2xl flex items-center p-2 focus-within:border-emerald-400 focus-within:shadow-[0_0_15px_rgba(16,185,129,0.3)] transition-all">
                            <input 
                                type="file" 
                                id="chat-file-marketing"
                                onChange={handleChatFileChange}
                                className="hidden"
                                accept="image/*,application/pdf"
                            />
                            <label 
                                htmlFor="chat-file-marketing" 
                            ...
                            <input 
                                ...
                                placeholder="Escribile a Ari de Marketing..."
