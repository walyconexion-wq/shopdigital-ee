...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ children, roles }) => {
    const { user, role, status, loading, login, loginAsDirector } = useAuth();
            ...
            <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4">
                <div className="bg-zinc-900 border border-white/10 p-8 rounded-[2.5rem] w-full max-w-sm text-center shadow-2xl relative overflow-hidden">
                    ...
                    <button 
                        ...
                        className="w-full bg-white text-black py-4 rounded-xl font-bold flex items-center justify-center gap-3 hover:bg-gray-200 transition-colors shadow-lg cursor-pointer"
                    >
                    ...
                    {/* 🧪 MODO LABORATORIO / EMERGENCIA: Acceso Directo del Director */}
                    <button
                        onClick={() => {
                            playNeonClick();
                            loginAsDirector();
                        }}
                        className="w-full mt-3 py-2.5 px-3 rounded-xl bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 text-amber-300 font-extrabold text-[9px] uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                        <span>⚡ Acceso Rápido Director (Waly Root)</span>
                    </button>
                </div>
            </div>
        );
