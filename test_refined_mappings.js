...
async function testRefinedMappings() {
  try {
    ...
    // 2. Find Module 2 and modify its mapper & filter
    const mod2 = bp.flow.find(m => m.id === 2);
    if (!mod2) throw new Error("Module 2 not found");
    
    // Remove system_instruction completely (setting to undefined)
    delete mod2.mapper.system_instruction;
    
    // Update text input to use first()
    if (mod2.mapper.contents && mod2.mapper.contents[0] && mod2.mapper.contents[0].parts && mod2.mapper.contents[0].parts[0]) {
      mod2.mapper.contents[0].parts[0].text = "{{first(1.entry[].changes[].value.messages[].text.body)}}";
    }

    // Set filter to check if messages text body exists
    mod2.filter = {
      label: "Only on message",
      conditions: [
        [
          {
            a: "{{first(1.entry[].changes[].value.messages[].text.body)}}",
            o: "exists"
          }
        ]
      ]
    ...
    // 3. Find Module 5 and update its `to` mapper using first()
    const mod5 = bp.flow.find(m => m.id === 5);
    if (!mod5) throw new Error("Module 5 not found");
    mod5.mapper.to = "{{first(1.entry[].changes[].value.messages[].from)}}";

    console.log("Patching scenario with refined mappings...");
                ...
                messages: [
                  {
                    ...
                    text: { body: "Hola, este es un mensaje de prueba definitivo" },
                    type: "text"
                  }
                ]
              },
